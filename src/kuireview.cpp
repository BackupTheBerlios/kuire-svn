...
KuireView::KuireView(QWidget *parent)
    : QWidget(parent),
      DCOPObject("KuireIface")
{
...
void KuireView::print(QPainter *p, int height, int width)
{
    // do the actual printing, here
    // p->drawText(etc..)
}

QString KuireView::currentURL()
{
    return m_html->url().url();
}

void KuireView::openURL(QString url)
{
    openURL(KURL(url));
}

void KuireView::openURL(const KURL& url)
{
    m_html->openURL(url);
}

void KuireView::slotOnURL(const QString& url)
{
    emit signalChangeStatusbar(url);
}

void KuireView::slotSetTitle(const QString& title)
{
    emit signalChangeCaption(title);
}
#include "kuireview.moc"
